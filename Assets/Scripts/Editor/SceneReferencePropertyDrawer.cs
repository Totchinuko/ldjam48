...
namespace Constantine
{
    ...
    public class SceneReferencePropertyDrawer : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            int lines = property.isExpanded ? sceneAsset != null ? 3 : 2 : 1;
        ...
        private static SceneAsset GetSceneAssetProperty(SerializedProperty property)
        {
            return AssetDatabase.LoadAssetAtPath<SceneAsset>(property.FindPropertyRelative(scenePathPropertyString).stringValue);
        }

        private static void SetSceneAssetProperty(SerializedProperty property, SceneAsset asset)
        {
            if (asset == null)
                property.FindPropertyRelative(scenePathPropertyString).stringValue = "";
            else
                property.FindPropertyRelative(scenePathPropertyString).stringValue = AssetDatabase.GetAssetPath(asset);
        }
        ...
        private static class BuildUtils
        {
            ...
            public static void AddBuildScene(BuildScene buildScene, bool force = false, bool enabled = true)
            {
                if (force == false)
                {
                    float selection = EditorUtility.DisplayDialogComplex(
                        "Add Scene To Build",
